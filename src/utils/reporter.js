...
function configAnalyser(configs) {
	const hasPerformanceBudget = !config.performance || Object.keys(configs.performance).length < 1
...
function _formattedSuccessfulRun(stats) {
	if (stats?.errors?.length || stats?.warnings?.length) return []
...
export { configAnalyser, statsReporter }
